//--------------------------------------------------------------------------------
//	$Id: time_domain.js,v 1.58 2013/08/13 17:48:45 wolf Exp wolf $
//--------------------------------------------------------------------------------
...
var TimeDomain = {
	...
	evaluateTimesForDays: function (request) {
		...
		//	24/7
		//------------------------------------------------------------
		//	24/7 is a special case, which can be rendered easily
		//	to an offline or paper map, and should not be watered
		//	down by notations like “24/7; Mo 12:00-14:00 off”.
		//------------------------------------------------------------
				...
				case '[':
					...
					for (;;) {
						...
						case '-':
							...
							var snth = this.G(request.tokens);
							if (snth<1 || snth>5)
								return 'Number between 1 and 5 expected';
							var start = request.l - snth*7;
	...
	scan: function (request, expression) {
		...
		//--------------------------------------------------------------------
		//
		//	The time domain expression is parsed into tokens:
		//
		//	Words:		Jan  January  Mo  Monday  open  sunset
		//	Always open:	24/7
		//	Time:		06:30	20:00
		//	Calendar day:	1.	2.	31.
		//	Number:		1	2	3
		//	String:		"on appointment"
		//			(strings may not contain '"' or ';')
		//	Alternative:	||
		//	Garbage:	(sequence of characters which are neither
		//			alphanumeric nor blankspace)
		//
		//--------------------------------------------------------------------
		//	The match method returns a list of tokens.
		//--------------------------------------------------------------------

		var symbols = expression.match (
			/[a-z_]+|24\/7|[0-9]+[.:][0-9]+|[0-9]+[.]|[0-9]+|"[^"]*"|\|\||[^\w\s]/gi);
		...
		for (i in symbols) {
			...
			if (!isNaN(value)) {
				...
				var l2 = token.split(token.match(/[.]/) ? '.' : ':')[1];
				var value2 = parseInt (l2);

				//----------------------------------------------------
				//	Time notation “HH:MM” or “HH.MM”
				//----------------------------------------------------

				if (!isNaN(value2)) {

					//--------------------------------------------
					//	Verify hours<36 and minutes<60?
					//--------------------------------------------

					if (token.length>5 || l2.length!=2 || value>36 || value2>=60)
					this.scanError (request,
						'Invalid time at "'+token+'" in rule "'+request.rule+'"');

					request.parsedExpression.push ({t:'t', v:value*60+value2, s:token});
					continue;
				}
				...
				if (token.split('.').length>=2) {
					...
					if (value<1 || value>31)
					this.scanError (request,
						'Invalid day at "'+token+'" in rule "'+request.rule+'"');

					request.parsedExpression.push ({t:'d', v:value, s:token});
					continue;
				}

				//----------------------------------------------------
				//	Always notation “24/7”
				//----------------------------------------------------

				if (token=='24/7') {
					request.parsedExpression.push ({t:token, s:token});
					continue;
				}

				//----------------------------------------------------
				//	Number less or equal 365: day of year
				//----------------------------------------------------

				if (value<=365) {
					request.parsedExpression.push ({t:'n', v:value, s:token});
					continue;
				}

				//----------------------------------------------------
				//	Number in “1970”…“2070”: year
				//----------------------------------------------------

				if (value>=1970 && value<2070) {
					request.parsedExpression.push ({t:'y', v:value, s:token});
					continue;
				}
			}
			...
			for (var msg in this.wrong_words) {
				...
				//----------------------------------------------------
				//	get replacement for bad word
				//----------------------------------------------------

				replacement = this.wrong_words[msg][lower];
				...
				this.scanError (request,
					msg.replace(/<ko>/,token).replace(/<ok>/,replacement));

				//----------------------------------------------------
				//	if replacement by empty string, discard token
				//----------------------------------------------------

				lower=replacement.toLowerCase();
				if (lower=='') continue sym;
			...
			case '[':
			case ']':
				request.parsedExpression.push ({t:lower, s:token});
				continue;
			case '–':
				this.scanError (request,
					'Use a minus sign "-" instead of a dash "–".');
				request.parsedExpression.push ({t:'-', s:token});
				continue;
			case ';':
				request.parsedExpression.push ({s:token});
				continue;
			case '||':
				request.parsedExpression.push ({s:token});
				continue;
			...
			if ((value=this.seasons[lower]) != null) {
				request.parsedExpression.push ({t:'s', v:value, s:token});
				continue;
			}
			if ((value=this.months[lower]) != null) {
				request.parsedExpression.push ({t:'m', v:value, s:token});
				continue;
			}
			if ((value=this.weekdays[lower]) != null) {
				request.parsedExpression.push ({t:'w', v:value, s:token});
				continue;
			}
			if ((value=this.movables[lower]) != null) {
				request.parsedExpression.push ({t:'v', v:value, s:token});
				continue;
			}
			if ((value=this.holidays[lower]) != null) {
				request.parsedExpression.push ({t:'h', v:value, s:token});
				continue;
			}
			if ((value=this.events[lower]) != null) {
				request.parsedExpression.push ({t:'e', v:value, s:token});
				continue;
			}
			...
			if (token.substr(0,1)=='"') {
				request.parsedExpression.push ({t:'"', v:this.T(token.substr(1,token.length-2)), s:token});
				continue;
			}
			...
			for (var msg in this.wrong_weekdays) {
				value = this.wrong_weekdays[msg][lower];
				if (value==null) continue;
				var ok = ['Su','Mo','Tu','We','Th','Fr','Sa'][value];
				this.scanError (request,
					msg.replace(/<ko>/,token).replace(/<ok>/,ok));
				request.parsedExpression.push ({t:'w', v:value, s:token});
				continue sym;
			}

			for (var msg in this.wrong_months) {
				value = this.wrong_months[msg][lower];
				if (value==null) continue;
				var ok = [null,'Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][value];
				this.scanError (request,
					msg.replace(/<ko>/,token).replace(/<ok>/,ok));
				request.parsedExpression.push ({t:'m', v:value, s:token});
				continue sym;
			...
			for (var msg in this.wrong_seasons) {
				value = this.wrong_seasons[msg][lower];
				if (value==null) continue;
				var ok = [null,'Spring','Summer','Autumn','Winter'][value];
				this.scanError (request,
					msg.replace(/<ko>/,token).replace(/<ok>/,ok));
				request.parsedExpression.push ({t:'s', v:value, s:token});
				continue sym;
